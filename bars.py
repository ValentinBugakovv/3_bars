...
def get_biggest_bar(bar_data):
    biggest_bar = max(bar_data['features'], key=lambda bar:
...
def get_smallest_bar(bar_data):
    smallest_bar = min(bar_data['features'], key=lambda bar:
    bar['properties']['Attributes']['SeatsCount'])
...
def get_closest_bar(bar_data, longitude=37.55, latitude=55.75):
    closest_bar = min(bar_data['features'], key=lambda bar: sqrt(
